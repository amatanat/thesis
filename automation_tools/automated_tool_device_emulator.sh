...
device_serialno=$(adb devices | sed -n "2p"| cut -f1)
echo $device_serialno

source config_2.conf
...
wait_device_screen () {
	...
	while true
	do 
	  if [ "$(adb -d shell dumpsys nfc | grep 'mScreenState=')" == "mScreenState=ON_UNLOCKED" ]; then 
		break
	  fi
	done
...
extract_fs_metadata () {
	...
	until [ $(adb devices | grep -c 'recovery') != 0 ]; do 
		sleep 1s
		if [ $(adb devices | grep -c 'device') == 3 ]; then
	...
	adb -d forward tcp:8992 tcp:8992 
...
do	
	...
	./$python_script_name $device_serialno
