...
def insert_action_fingerprints (db, app_name, action_name, key, changed, inode): 
	sql_insert = "INSERT INTO action_fingerprints (app_name, action_name, key, changed, inode) VALUES(?,?,?,?,?)"
   	cursor = db.cursor()
...
if __name__ == '__main__':
	...
	sql_query =  """SELECT count(*), app_name, inode, changed, action_name, key 
		        FROM action_profiles
		        WHERE action_profiles.action_name = ? 
			AND action_profiles.app_name = ?
			GROUP BY inode, changed
			HAVING count (*) = (
				SELECT MAX(run)
	...
	if db is not None:
		...
		for row in rows:
			insert_action_fingerprints (db, app_name, action_name, row[5], row[3], row[2])
		db.commit()
